...
print(sys.executable)  # 打印执行的编译器路径
...
videopath = '1.mp4'  # 视频文件路径
# Directory to store the processed frames
dir = './pic'  # 存放路径
# smoothing window size
len_window = int(10)  # 平滑窗口
...
print("Video :" + videopath)  # 打印视频的路径
print("Frame Directory: " + dir)  # 存放帧的路径

cap = cv2.VideoCapture(str(videopath))  # 视频的读取

curr_frame = None  # 前后帧作对比
prev_frame = None
...
while (ret):
    ...
    curr_frame = luv
    if curr_frame is not None and prev_frame is not None:  # 如果当前帧和先前的帧都存在
        # logic here
        diff = cv2.absdiff(curr_frame, prev_frame)  # 计算先前帧和当前帧的差异
        count = np.sum(diff)  # 计算差异
