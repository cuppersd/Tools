...
def show_pic(img, bboxes=None, name='pic'):
    ...
    cv2.imwrite(str(uuid.uuid1())+'1.jpg', show_img)
...
class DataAugment():
    ...
    def resize_pic(self, im, text_polys, input_size):
        h, w, c = im.shape
        long_edge = max(h, w)
        if long_edge > input_size:
            scale = input_size / long_edge
            im = cv2.resize(im, dsize=None, fx=scale, fy=scale)
        else:
            scale = 1
        ...
        h_, w_, c_ = im.shape
        print(h_, w_, c_)
        start_w = random.choice([0] + list(range(input_size - w_)))
        start_h = random.choice([0] + list(range(input_size - h_)))
        print(start_w, start_h)
        im_padded = np.ones((input_size, input_size, c), dtype=np.uint8) * 127
        im_padded[start_h:h_ + start_h, start_w:w_ + start_w] = im.copy()
        text_polys = text_polys * scale
        text_polys[:, :, 0] += start_w
        text_polys[:, :, 1] += start_h
        return im_padded, text_polys
    ...
    def test(self, im: np.ndarray, text_polys: np.ndarray):
        ...
        ss, dd = self.resize_pic(t_im, t_text_polys, 736)
        print(dd)
        show_pic(ss, dd, 'add_noise')
...
aug = DataAugment()
img = cv2.imread("D:/MY_EAST/data/train/img/1533996138.813304.jpg")
boxxx=[]
f = open("D:/MY_EAST/data/train/txt/1533996138.813304.txt",'r',encoding='utf-8').readlines()
for i in f:
    boxxx.append(np.array([float(x) for x in i.split(',')[:8]]).reshape(4,2))

# ss,dd= resize_pic(img, np.array(boxxx), 736)
#
# show_pic(ss, dd, 'add_noise')
